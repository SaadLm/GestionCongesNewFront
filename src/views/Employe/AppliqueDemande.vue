...
export default {
    ...
    computed:{
        ...
        minDate() {
            ...
            // }
            // else{
            //     const dateStringFin = new Date(this.demande.dateFin);
            //     const min = new Date(dateStringFin);
            //     const addDays = - Math.floor(this.nbr)-1;
            //     min.setDate(dateStringFin.getDate() + addDays);
            //
            //     const formaDate = min.toISOString().split("T")[0];
            //     return formaDate;
            //
            // }
        },
    ...
    methods:{
        ajouterDemande(){
            ...
            this.demande.id_e=this.employe.id_e
            ...
            if(e>=s){
                ...
                if (this.demande.dateDebut && this.demande.dateFin) {
                    ...
                    if(differenceInDays>Math.floor(this.employe.soldeConge)){
                        this.err=" Solde insuffisant"
                    }
                    else{
                        if(this.demande.motif=='' ||this.demande.motif==null || this.demande.dateDebut ==null|| this.demande.dateFin==null){
                          ...
                          const demande={
                            ...
                            axios.post('http://localhost:3000/demande',demande)
                                .then(response=>{
                                    this.showSuccessMessage('demande ajouter avec succes')
                                    console.log('la demande a ajouté avec succes',response)
                                })
                                .catch(err=>{
                                    console.log(err)
                                })
                ...
                this.err='Date de Debut ne peut pas etre superieur que date fin'
            }
        ...
        calcMaximum() {
            ...
            //
            // const dateStringDebut = new Date(this.demande.dateDebut);
            //
            // if (isNaN(dateStringDebut)) {
            //     // Handle the case where this.demande.dateDebut is not a valid date
            //     return "Invalid date";
            // }
            //
            // const max = new Date(this.demande.dateDebut);
            // const addDays = Math.floor(this.nbr)-1;
            // max.setDate(dateStringDebut.getDate() + addDays);
            //
            // this.calcMax= max.toISOString().split("T")[0];
        }
        ,
        updateMaxDate() {
            // Update maxDate
            console.log('na khdama')
            this.calcMaximum()
        },
        ...
        refreshSolde(){
            const obj={id_e:this.employe.id_e}
            console.log(obj.id_e)
            axios.post('http://localhost:3000/getsolde',obj)
                .then(response=>{
    ...
    mounted() {
        ...
        // this.nbr=this.employe.soldeConge;
...
<!--                                <div>-->
<!--                                    <label  class="formbold-form-label"> Date de début : </label>-->
<!--                                    <input-->
...
<!--                                </div>-->
                                <div class="row w-100">
                                    <!--                                <input v-model="demande.dateDebut" type="date" data-date="" data-date-format="DD MMMM YYYY" @change="updateMaxDate" @input="handleChange" class="form-control form-control-sm" />-->
                                    <!--                                    <div>-->
...
<!--                                        <span>{{ demande.dateDebut}}</span>-->
                                    ...
                                    <div class="col-md-5 offset-md-1">
                                        ...
                                        <label  class="formbold-form-label"> Date fin : </label>
                                      <flat-pickr
...
<!--                                />-->
                          ...
                          <div class="formbold-form-step-1 active">
                            ...
                            <label  class="formbold-form-label"> Motif !: </label>
                              <div class="row w-100">
<!--                                  <div class="col-md-12 offset-md-0 col-sm-11 offset-sm-1">-->
                                    <input
                                            v-model="demande.motif"
                                            type="text"
                                            placeholder="Congé mariage, Congé de naissance, ... "
                                            class="formbold-form-input me-0"
                                    />
<!--                                  </div>-->
                              </div>
                          </div>
                        ...
                        <div class="formbold-form-btn-wrapper"><span class="text-danger fw-bolder">{{ err}}</span>
                            <span @click="ajouterDemande" class="formbold-btn">
                                Confirmer
                            </span>
                        </div>
